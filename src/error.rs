...
mod tests {
    ...
    fn test_error_display() {
        let errors = vec![
            (UiError::invalid_config("Button", "Invalid type"), "Invalid configuration for 'Button': Invalid type"),
            (UiError::InvalidPlatform("unknown".to_string()), "Invalid platform: unknown"),
            (UiError::missing_property("Card", "title"), "Missing required property 'title' for 'Card'"),
            (UiError::invalid_value("Input", "type", "invalid", "text, number, password"), "Invalid value 'invalid' for property 'type' in 'Input'. Expected: text, number, password"),
            (UiError::render("Failed to render HTML"), "Render error: Failed to render HTML"),
            (UiError::Generic("Something went wrong".to_string()), "Error: Something went wrong"),
        ];
